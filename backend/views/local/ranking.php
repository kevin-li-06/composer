...
<script>
    ...
    var bigchance = echarts.init(document.getElementById('bigchance'));
...
//            data: ['2011年', '2012年']
        ...
        yAxis: {
            ...
            type: 'category',
            data: [
                <?php foreach ($big_chancedata as $key=>$chancedata){ echo '"'.$chancedata['zh_storename'].'"'.',';}?>
                ]
        },
        series: [
            {
                name: '大奖机会的次数',
                type: 'bar',
                data: [<?php foreach ($big_chancedata as $chancedata){ echo isset($chancedata['big_num']) ? $chancedata['big_num'].',' : 0 . ',';}?>]
            },
        ]
    };
    //     使用刚指定的配置项和数据显示图表。
