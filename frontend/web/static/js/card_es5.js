...
var lottery = function() {
    ...
    function lottery(id, coverUrl, container, prizeList, ajaxUrl, user_id, callback) {
    ...
    _createClass(lottery, [{
            key: 'initialCover',
            value: function initialCover() {

                    this.ctx.save();
                    var _this = this
                    var img = new Image();
                    img.src = _this.coverUrl.url;
                    img.onload = function() {
            ...
            key: 'drawBg',
            value: function drawBg(lvl, list) {
                    //设置字体样式
                    $(this.c1).after("<div style='position:absolute;left:50%;top:50%;font-size:3rem;z-index:0;transform: translate(-50%,-50%)'>" + list['lv' + lvl][0] + "</div>");
                    this.delay = true;
                }
                //添加事件监听
                ...
                if (!this.isRequest) {
                    ...
                    $.ajax({
                        ...
                        success: function success(data) {
                            //console.log('success', data); // 临时test
                            var d = data
                            if (d.status == 'success') {
                                var i = null;
        ...
        {
            key: 'getTransparentPercent',
            value: function getTransparentPercent(ctx, width, height) {
                //刮开百分比计算
        ...
        {
            key: 'init',
            value: function init(code, prizeLvl, prizeList, prizeUsedList) {
                // 重置canvas的宽高
